...
export function App() {
  // Determine if there is an active session from previous login/signup
  const initialSessionEmail = getActiveSessionEmail();
  const initialUserData = initialSessionEmail ? getUserDataByEmail(initialSessionEmail) : null;
  const initialAuth = Boolean(initialSessionEmail);

  // Auth state: false if not logged in / signed out / not signed up yet
  ...
  // Domain state initialized from active user session if available
  const [profile, setProfile] = useState<UserProfile>(
    initialUserData?.profile || getStoredProfile()
  );
  const [accounts, setAccounts] = useState<UserAccount[]>(
    initialUserData?.accounts || getStoredAccounts()
  );
  const [transactions, setTransactions] = useState<Transaction[]>(
    initialUserData?.transactions || getStoredTransactions()
  );
  const [goals, setGoals] = useState<FinancialGoal[]>(
    initialUserData?.goals || getStoredGoals()
  );
  const [aiReport, setAiReport] = useState<AiFinancialReport | null>(
    initialUserData?.aiReport !== undefined ? initialUserData.aiReport : getStoredReport()
  );

  // Currency
  const [currency, setCurrencyState] = useState<CurrencyCode>(
    initialUserData?.currency || getStoredCurrency() || profile.preferredCurrency || 'INR'
  );
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 font-sans antialiased transition-colors duration-200">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16">
        {!isAuthenticated ? (
          <LandingPage
            onStartDemo={() => {
              const demoEmail = 'alex.morgan@fintech.ai';
              setActiveSessionEmail(demoEmail);
      ...
      <AuthModal
        ...
        onAuthenticate={(name, email, isDemo) => {
          ...
          setActiveSessionEmail(cleanEmail || (isDemo ? 'alex.morgan@fintech.ai' : 'user@fintech.ai'));
          setIsAuthenticated(true);
