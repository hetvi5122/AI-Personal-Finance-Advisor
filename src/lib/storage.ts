...
const STORAGE_KEYS = {
  ...
  ACTIVE_SESSION: 'fin_ai_active_session_v1',
...
export function getActiveSessionEmail(): string | null {
  const email = safeGetItem<string | null>(STORAGE_KEYS.ACTIVE_SESSION, null);
...
export function setActiveSessionEmail(email: string | null): void {
  if (!email || !email.trim()) {
    localStorage.removeItem(STORAGE_KEYS.ACTIVE_SESSION);
    safeSetItem(STORAGE_KEYS.AUTH, false);
  } else {
    const clean = email.trim().toLowerCase();
    safeSetItem(STORAGE_KEYS.ACTIVE_SESSION, clean);
